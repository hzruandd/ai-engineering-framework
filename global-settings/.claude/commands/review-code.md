...
- [ ] 更新数据后清理缓存
- [ ] 缓存穿透、击穿、雪崩防护
- [ ] **缓存key长度检查**：结合上下文逻辑，检查拼接后的key总长度是否合理（建议不超过256字节），避免动态拼接导致key过长占用额外内存
- [ ] **缓存value大小检查**：结合业务上下文，评估缓存对象序列化后的大小是否合理（单个value建议不超过1MB，String类型建议不超过10KB），大对象应拆分或精简字段后再缓存
- [ ] **缓存过期时间检查**：每个`setCacheObject`/`setCacheList`调用必须显式设置过期时间；如果业务上确实不需要过期（如全局配置），必须在代码旁添加注释说明原因，例如：`// 全局配置数据，随服务生命周期，无需过期`
