...
```text
重点看这次改动里的脱敏、加密、密钥和租户隔离风险，tapd=TAPD-123456 mode=security-only
```

## 示例 7：缺少 TAPD 时的预评审

```text
先做一次预评审，重点扫当前工作区风险，mode=precheck
```

正确响应：

```text
可以先做预评审，但这次只输出风险与待补证据项，不给放行结论。进入正式评审前仍需补 TAPD 编号和对应变更目标。
```

## 示例 8：缺少 TAPD 时的正式门禁交互
